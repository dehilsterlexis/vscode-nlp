...
export enum reformatType { NORMAL, ONELINE, MATCH }
...
export class NLPFile extends TextFile {
	...
	formatRule(ruleStr: string, type: reformatType = reformatType.NORMAL): string {
		...
		for (let i=0; i < ruleStr.length; i++) {
			...
			// If you have one node followed immediately by another or a new line
				...
				this.constructLine(rules,words);
				words = [];
				word = '';
				currentState = state.NODE;
		...
		for (let rule of rules) {
			if (rule.rule == '@@') {
				ruleLine = type == reformatType.NORMAL ? '\t@@' : '@@';
				hasAtAt = true;
		...
		var sep = type == reformatType.NORMAL ? this.getSeparator() : '';
	...
	constructLine(rules, words: string[])  {
		...
		rules.push({suggested: '', rule: line, comment: userComment});
	}
